...
dataset <- read.csv("C:\\Users\\chenz\\OneDrive\\Desktop\\Fall 2022\\IST 421 - Information Visualization\\project\\leetcode_questions.csv")
...
ggplot(df.level, aes(ymax=ymax, ymin=ymin, xmax=2, xmin=1, fill=level)) +
  geom_rect() +
  geom_text(x=1.5, aes(y=labelPosition, label=label), size=3.5) +
...
wordcloud(words = df.topic$word, freq = df.topic$count, min.freq = 1,
...
# box plot - 3
dataset$Difficulty.Level <- factor(dataset$Difficulty.Level, levels=c("Easy", "Medium", "Hard"))
...
df.like %>%
  ...
  coord_flip() +
  theme_tufte() + 
  geom_text(aes(label=Likes), position=position_dodge(width=10), vjust=-1) + 
  xlab("")
...
df.hash <- dataset %>% filter(grepl('Hash Table', Topic.Tagged.text))
...
df.dp <- dataset %>% filter(grepl('Dynamic Programming', Topic.Tagged.text))
df.dp <- as.data.frame(table(df.dp$Difficulty.Level))
colnames(df.dp) <- c("difficulty", "count")
df.dp$tag <- c("dynamic programming", "dynamic programming", "dynamic programming")
...
df.dfs <- dataset %>% filter(grepl('Depth-First Search', Topic.Tagged.text))
df.dfs <- as.data.frame(table(df.dfs$Difficulty.Level))
colnames(df.dfs) <- c("difficulty", "count")
df.dfs$tag <- c("depth-first search", "depth-first search", "depth-first search")


df.sorting <- dataset %>% filter(grepl('Sorting', Topic.Tagged.text))
...
df.bfs <- dataset %>% filter(grepl('Breadth-First Search', Topic.Tagged.text))
df.bfs <- as.data.frame(table(df.bfs$Difficulty.Level))
colnames(df.bfs) <- c("difficulty", "count")
df.bfs$tag <- c("breadth-first search", "breadth-first search", "breadth-first search")


df.5 <- rbind(df.array, df.string, df.hash, df.dp, df.math, df.dfs, df.sorting, df.greedy, df.db, df.bfs)

ggplot(df.5) + aes(x = tag, y = count, fill = factor(difficulty, levels = c("Hard", "Medium", "Easy")), label = count) + 
  geom_bar(stat = "identity", color = "black") +
  scale_fill_manual(values=c("#f6a0a1", "#f9d5a5", "#b1daae")) + 
  theme_tufte() + 
  theme(legend.position="top") + 
  scale_x_discrete(limits=c("array", "string", "hash table", "dynamic programming", "math", "depth-first search", "sorting", "greedy", "database", "breadth-first search")) +
...
df.arrayy <- aggregate(df.array2$total.submission,
...
df.stringg <- aggregate(df.string2$total.submission,
                       list(tag = df.string2$tag,
                            reg = df.string2$Difficulty.Level),
                       sum)


df.hash2 <- dataset %>% filter(grepl('Hash Table', Topic.Tagged.text))
df.hash2$tag <- c(rep("hash table", 372))
df.hashh <- aggregate(df.hash2$total.submission,
                        list(tag = df.hash2$tag,
                             reg = df.hash2$Difficulty.Level),
                        sum)


df.dp2 <- dataset %>% filter(grepl('Dynamic Programming', Topic.Tagged.text))
...
df.mathh <- aggregate(df.math2$total.submission,
...
ggplot(df.6) + aes(area = x, fill = reg, subgroup = reg) + 
  geom_treemap() +
  geom_treemap_subgroup_text(color = "white", place = "topleft") +
  geom_treemap_text(aes(label = tag), color = "black", place = "centre", grow = T) +
  scale_fill_manual(values=c("#f6a0a1", "#f9d5a5", "#b1daae")) +
